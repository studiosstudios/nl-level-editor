...
const action = tiled.registerAction("LaunchSymbiosis", function (action) {
    if (tiled.activeAsset.isTileMap) {
        ...
        const parentDirectory = tiled.extensionsPath + "/TiledExtension/";
        ...
        try {
            ...
            const writeResult = tiled.mapFormat("json").write(tiled.activeAsset as TileMap, tempFilePath);
...
function startLauncher(parentDirectory: string, tempFilePath: string) {
...
function startConfig(parentDirectory: string) {
